...
/*
 * MORI v2.6.0 short-lived correlation state.
 *
 * Key:   process TGID
...
/*
 * MORI v2.6.0 short-lived splice state.
 *
 * Key:   process TGID
             ...
             int ret)
{
    ...
    if (aead_recent && splice_recent) {
        /*
         * MORI v2.6.0 selective enforcement.
         *
         * The combined recent AEAD + SUID-to-pipe splice context
         * causes this matching SUID read permission check to fail.
         *
         * Consume only the current splice state. AEAD state remains
         * active so another qualifying splice can be evaluated again.
         */
        bpf_map_delete_elem(&splice_state, &tgid);

        bpf_printk(
            "MORI v2.6.0 DENY-AEAD-SPLICE-SUID tgid=%u uid=%u",
            tgid,
            uid
        );
    ...
    if (aead_recent) {
        bpf_printk(
            "MORI v2.6.0 CORRELATED-AEAD-SUID tgid=%u uid=%u",
            tgid,
            uid
        );
    ...
    if (splice_recent) {
        bpf_printk(
            "MORI v2.6.0 CORRELATED-SPLICE-SUID tgid=%u uid=%u",
            tgid,
            uid
        );
    ...
    bpf_printk(
        "MORI v2.6.0 READ-SUID tgid=%u uid=%u mask=%d",
        tgid,
        uid,
        mask
    );
             ...
             int ret)
{
    ...
    if (mori_sockaddr_type_is_aead(address, addrlen)) {
        ...
        if (state_err == 0) {
            bpf_printk(
                "MORI v2.6.0 AEAD-ARM tgid=%u uid=%u len=%d",
                tgid,
                uid,
                addrlen
            );
            ...
            bpf_printk(
                "MORI v2.6.0 AEAD-STATE-FAIL tgid=%u uid=%u",
                tgid,
                uid
            );
        }
        ...
        bpf_printk(
            "MORI v2.6.0 AF_ALG-OTHER tgid=%u uid=%u len=%d",
            tgid,
            uid,
            addrlen
        );
             ...
             unsigned int flags)
{
    ...
    if (state_err == 0) {
        bpf_printk(
            "MORI v2.6.0 SPLICE-ARM tgid=%u uid=%u len=%lu",
            tgid,
            uid,
            len
        );
        ...
        bpf_printk(
            "MORI v2.6.0 SPLICE-STATE-FAIL tgid=%u uid=%u",
            tgid,
            uid
        );
    }
