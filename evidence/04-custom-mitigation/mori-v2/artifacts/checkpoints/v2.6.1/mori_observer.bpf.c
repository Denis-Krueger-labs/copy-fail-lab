...
/*
 * MORI v2.6.1 short-lived correlation state.
 *
 * Key:   process TGID
...
/*
 * MORI v2.6.1 short-lived splice state.
 *
 * Key:   process TGID
...
/*
 * MORI v2.6.1 structured deny telemetry.
 *
             ...
             int ret)
{
    ...
    if (aead_recent && splice_recent) {
        ...
        /*
         * MORI v2.6.1 selective enforcement + structured telemetry.
         *
         * Event delivery is best-effort only. Failure to reserve a
         * ring-buffer record must never weaken the enforcement decision.
         */
        ...
        bpf_printk(
            "MORI v2.6.1 DENY-AEAD-SPLICE-SUID tgid=%u uid=%u",
            tgid,
            uid
        );
    ...
    if (aead_recent) {
        bpf_printk(
            "MORI v2.6.1 CORRELATED-AEAD-SUID tgid=%u uid=%u",
            tgid,
            uid
        );
    ...
    if (splice_recent) {
        bpf_printk(
            "MORI v2.6.1 CORRELATED-SPLICE-SUID tgid=%u uid=%u",
            tgid,
            uid
        );
    ...
    bpf_printk(
        "MORI v2.6.1 READ-SUID tgid=%u uid=%u mask=%d",
        tgid,
        uid,
        mask
    );
             ...
             int ret)
{
    ...
    if (mori_sockaddr_type_is_aead(address, addrlen)) {
        ...
        if (state_err == 0) {
            bpf_printk(
                "MORI v2.6.1 AEAD-ARM tgid=%u uid=%u len=%d",
                tgid,
                uid,
                addrlen
            );
            ...
            bpf_printk(
                "MORI v2.6.1 AEAD-STATE-FAIL tgid=%u uid=%u",
                tgid,
                uid
            );
        }
        ...
        bpf_printk(
            "MORI v2.6.1 AF_ALG-OTHER tgid=%u uid=%u len=%d",
            tgid,
            uid,
            addrlen
        );
             ...
             unsigned int flags)
{
    ...
    if (state_err == 0) {
        bpf_printk(
            "MORI v2.6.1 SPLICE-ARM tgid=%u uid=%u len=%lu",
            tgid,
            uid,
            len
        );
        ...
        bpf_printk(
            "MORI v2.6.1 SPLICE-STATE-FAIL tgid=%u uid=%u",
            tgid,
            uid
        );
    }
