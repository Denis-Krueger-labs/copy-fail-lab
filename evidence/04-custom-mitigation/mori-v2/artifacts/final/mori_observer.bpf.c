...
/*
 * MORI v2.6.2 short-lived correlation state.
 *
 * Key:   process TGID
...
/*
 * MORI v2.6.2 short-lived splice state.
 *
 * Key:   process TGID
...
/*
 * MORI v2.6.2 structured deny telemetry.
 *
             ...
             int ret)
{
    ...
    if (aead_recent && splice_recent) {
        ...
        /*
         * MORI v2.6.2 selective enforcement + structured telemetry.
         *
        ...
        bpf_printk(
            "MORI v2.6.2 DENY-AEAD-SPLICE-SUID tgid=%u uid=%u",
            tgid,
            uid
        );
    ...
    if (aead_recent) {
        bpf_printk(
            "MORI v2.6.2 CORRELATED-AEAD-SUID tgid=%u uid=%u",
            tgid,
            uid
        );
    ...
    if (splice_recent) {
        bpf_printk(
            "MORI v2.6.2 CORRELATED-SPLICE-SUID tgid=%u uid=%u",
            tgid,
            uid
        );
    ...
    bpf_printk(
        "MORI v2.6.2 READ-SUID tgid=%u uid=%u mask=%d",
        tgid,
        uid,
        mask
    );
             ...
             int ret)
{
    ...
    if (mori_sockaddr_type_is_aead(address, addrlen)) {
        ...
        if (state_err == 0) {
            bpf_printk(
                "MORI v2.6.2 AEAD-ARM tgid=%u uid=%u len=%d",
                tgid,
                uid,
                addrlen
            );
            ...
            bpf_printk(
                "MORI v2.6.2 AEAD-STATE-FAIL tgid=%u uid=%u",
                tgid,
                uid
            );
        }
        ...
        bpf_printk(
            "MORI v2.6.2 AF_ALG-OTHER tgid=%u uid=%u len=%d",
            tgid,
            uid,
            addrlen
        );
             ...
             unsigned int flags)
{
    ...
    if (state_err == 0) {
        bpf_printk(
            "MORI v2.6.2 SPLICE-ARM tgid=%u uid=%u len=%lu",
            tgid,
            uid,
            len
        );
        ...
        bpf_printk(
            "MORI v2.6.2 SPLICE-STATE-FAIL tgid=%u uid=%u",
            tgid,
            uid
        );
    }
