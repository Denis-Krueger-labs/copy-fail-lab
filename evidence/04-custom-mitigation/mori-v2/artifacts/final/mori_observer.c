...
static int notify_process_tty(uint32_t tgid)
{
    static const int candidates[] = { 2, 1, 0 };
    ...
    for (i = 0; i < sizeof(candidates) / sizeof(candidates[0]); i++) {
        ...
        /*
         * Open the process descriptor independently, then verify that
         * the resulting object is genuinely a terminal before writing.
         */
        ...
        /*
         * Best-effort presentation only. Partial writes are handled,
         * but notification failure never affects enforcement.
         */
        while (remaining > 0) {
...
static int handle_mori_event(void *ctx, void *data, size_t len)
{
    const struct mori_event *event = data;

    (void)ctx;

    if (len < sizeof(*event)) {
        fprintf(stderr,
                "MORI: short ring-buffer event: %zu bytes\n",
                len);
        return 0;
    }

    if (event->type == MORI_EVENT_DENY) {
        printf("MORI event: DENY tgid=%u uid=%u\n",
               event->tgid,
               event->uid);
        fflush(stdout);

        if (notify_process_tty(event->tgid) == 0) {
            printf("MORI: attacker-facing TTY notification delivered to TGID %u.\n",
                   event->tgid);
        } else {
            printf("MORI: no interactive TTY available for TGID %u; "
...
int main(void)
{
    ...
    if (!rb) {
        fprintf(stderr,
                "MORI: failed to create ring buffer: %s\n",
                strerror(errno));
        mori_observer_bpf__destroy(skel);
        return 1;
    }

    printf("MORI v2.6.2 enforcement engine attached.\n");
    ...
    printf("Selective AEAD + SUID-to-pipe policy enforcement is active.\n");
    printf("Structured deny telemetry is active.\n");
    ...
    while (!exiting) {
        ...
        if (err < 0) {
            fprintf(stderr,
                    "MORI: ring buffer poll failed: %d\n",
                    err);
            break;
        }
    }
