...
\begin{evidencebox}
Phase~04 is preserved under \path{evidence/04-custom-mitigation/}. Its 126
verified public objects include the broad v1 control, all historical MORI v2
checkpoints, the frozen v2.7 source and build, and 67 screenshots. The outer
\path{PUBLIC-SHA256SUMS.txt} contains 126 entries, while the nested MORI v2
manifest contains 108. Both manifests verify without an error. The outer
manifest and the current Phase~04 \path{README.md} define the package used in
this chapter.
\end{evidencebox}
...
\begin{figure}[H]
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{04-custom-mitigation/mori-v2/screenshots/57-mori-v2-7-final-artifact-provenance.png}
    \caption{Later provenance capture for the transferred frozen v2.7 set.
    It verifies the executable and artifact manifest on
    \code{6.8.0-137-generic}; it is provenance evidence, not a replacement for
    the earlier mitigation-test chronology.}
    \label{fig:phase04-v27-artifact-provenance}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{04-custom-mitigation/screenshots/04-copyfail-blocked-by-mori-guard.png}
    \caption{Final MORI Guard v1 retest. The Python proof of concept stops at
    the unavailable AF\_ALG AEAD path, after which the same terminal records
    UID~1001, the known-good \code{/usr/bin/su} digest, and the still-blocked
    \code{algif\_aead} module.}
    \label{fig:phase04-v1-poc-blocked}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[
        width=0.96\textwidth,
        trim=0bp 335bp 260bp 0bp,
        clip,
        keepaspectratio
    ]{04-custom-mitigation/mori-v2/screenshots/29-mori-v2-5-shadow-policy-would-deny.png}

    \smallskip

    \includegraphics[
        width=0.76\textwidth,
        trim=615bp 2bp 0bp 300bp,
        clip,
        keepaspectratio
    ]{04-custom-mitigation/mori-v2/screenshots/29-mori-v2-5-shadow-policy-would-deny.png}
    \captionsetup{justification=raggedright,singlelinecheck=false}
    \caption{MORI v2.5 shadow-policy test. The kernel-side trace reaches
    \code{WOULD-DENY-AEAD-SPLICE-SUID}, while userspace still records a
    successful AEAD bind and splice because enforcement was not yet active.}
    \label{fig:phase04-v25-shadow-policy}
\end{figure}
...
The kernel-side programs make the enforcement decision. The principal state
maps and event channel are shown in \cref{tab:mori-v27-state}.
...
\begin{figure}[H]
    ...
    \includegraphics[
        width=0.82\textwidth,
        trim=864bp 3bp 0bp 207bp,
        clip,
        keepaspectratio
    ]{04-custom-mitigation/mori-v2/screenshots/50-mori-v2-6-2-same-tgid-expiry-bypass-reproduced.png}
    \captionsetup{justification=raggedright,singlelinecheck=false}
    \caption{Retained v2.6.2 regression capture. The same TGID keeps the AEAD
    socket open, waits 12~seconds, and then moves 32~bytes from the SUID target
    because the timed state has expired.}
    \label{fig:phase04-v262-expiry-bypass}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[
        width=0.72\textwidth,
        trim=864bp 4.5bp 0bp 255bp,
        clip,
        keepaspectratio
    ]{04-custom-mitigation/mori-v2/screenshots/52-mori-v2-7-expiry-bypass-regression-fixed.png}
    \captionsetup{justification=raggedright,singlelinecheck=false}
    \caption{The same delayed sequence against MORI v2.7. The trace reaches
    \code{DENY-AEAD-SPLICE-SUID}, userspace receives \code{EPERM}, and the
    optional terminal message is delivered.}
    \label{fig:phase04-v27-expiry-fixed}
\end{figure}
...
Phase~04 index records that the candidate was not rebuilt between the final
Python and independent C tests. The combined validation capture separately
shows one loader session receiving the deny events from both executions.
...
\begin{figure}[H]
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{04-custom-mitigation/mori-v2/screenshots/53-mori-v2-7-final-python-poc-denied.png}
    \caption{Final Python proof-of-concept validation with the frozen v2.7
    executable. The capture records the executable digest, matching kernel and
    userspace deny events, and the unprivileged process receiving
    \code{PermissionError} with \code{errno=1}.}
    \label{fig:phase04-v27-python-denied}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[
        width=0.92\textwidth,
        trim=864bp 338bp 0bp 207bp,
        clip,
        keepaspectratio
    ]{04-custom-mitigation/mori-v2/screenshots/54-mori-v2-7-final-python-post-validation.png}
    \caption{Highlighted post-test pane from the unmodified final Python
    validation screenshot. It records UID~1001 and the known-good
    \code{/usr/bin/su} digest after the denial; the complete capture remains in
    the Phase~04 evidence tree.}
    \label{fig:phase04-v27-python-post-state}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{04-custom-mitigation/mori-v2/screenshots/56-independent-c-poc-provenance.png}
    \caption{Successful provenance capture for the independent C test. It
    records the repository commit, source digest, and executable digest used in
    the validation.}
    \label{fig:phase04-independent-c-provenance}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.58\textwidth}
        ...
        \includegraphics[
            width=\linewidth,
            trim=864bp 112bp 0bp 270bp,
            clip,
            keepaspectratio
        ]{04-custom-mitigation/mori-v2/screenshots/55-mori-v2-7-final-c-poc-denied-and-post-validation.png}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.38\textwidth}
        \centering
        \includegraphics[
            width=\linewidth,
            trim=864bp 3.6bp 0bp 465bp,
            clip,
            keepaspectratio
        ]{04-custom-mitigation/mori-v2/screenshots/55-mori-v2-7-final-c-poc-denied-and-post-validation.png}
    \end{minipage}
    \captionsetup{justification=raggedright,singlelinecheck=false}
    \caption{Final independent C proof-of-concept validation. The lower-right
    session is shown in two highlighted panes: the first write fails, then the
    post-test check records UID~1001 and the unchanged target digest. The
    complete unmodified capture remains in the Phase~04 evidence tree.}
    \label{fig:phase04-v27-c-denied}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[
        width=0.82\textwidth,
        height=0.27\textheight,
        trim=768bp 0bp 0bp 0bp,
        clip,
        keepaspectratio
    ]{04-custom-mitigation/mori-v2/screenshots/16-mori-v2-2-benign-aead-bind-preserved.png}
    \caption{Highlighted right-hand pane of the unmodified v2.2 compatibility
    screenshot. The unprivileged control records \code{uid=1001} and
    \code{bind=success}; the complete capture also preserves the adjacent MORI
    observer output.}
    \label{fig:phase04-v22-benign-bind}
\end{figure}
