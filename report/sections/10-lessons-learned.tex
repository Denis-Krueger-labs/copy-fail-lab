...
behavioural integrity signal that was independent of the sample identity, but
that signal did not uniquely attribute Copy Fail. MORI v2.7 selectively denied
the two tested same-process exploitation paths while preserving the exercised
interface operations, but it retained explicit bypass, lifecycle, and fail-open
boundaries. The corrected vendor kernel produced the strongest operational
result because it removed the reproduced behaviour while leaving the tested
AF\_ALG interface available.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}>{\bfseries\raggedright\arraybackslash}p{0.09\textwidth}>{\raggedright\arraybackslash}p{0.44\textwidth}Y@{}}
        ...
        Repeatability was demonstrated on one reconstructed VM, not quantified
        across hosts, kernels, workloads, or timing conditions. \\
        ...
        RQ3 &
        Yes. MORI detected deviation from a known-good privileged target without
        matching the proof-of-concept filename, language, or exact sample hash. &
        ...
        RQ4 &
        Yes, within the tested model. MORI v2.7 denied both preserved
        proof-of-concept paths before privilege escalation while the exercised
        benign interface operations remained available. &
        ...
        RQ6 &
        Yes. On kernel \code{6.8.0-137-generic}, both tested implementations
        failed to reproduce the prior transition while the tested AF\_ALG
        operations remained reachable and the target retained its known-good
        state. &
        The retest was not a perfect single-variable trial and did not directly
        trace the corrected internal scatter-gather path. \\
        \bottomrule
    \end{tabularx}
\end{table}
...
backing-filesystem read, and recovery after reboot. The defensive phases then
show three distinct responses to that path: observing its consequence, denying
selected prerequisites or correlations, and removing the underlying vulnerable
behaviour through the kernel correction.
...
\code{/usr/bin/su}. That decision did not depend on whether the proof of concept
was written in Python or C, what the file was called, or whether a trivial byte
change altered its sample hash. In that narrow sense, the detector generalised
beyond the specific published artifact.
...
and negative MORI controls, the preserved sample hashes, and the patched-side
combination of interface availability with absent privilege transition. Phase
separation, VM snapshots, manifests, and explicit target-state checks made it
possible to relate those observations without treating every screenshot as a
...
\begin{enumerate}
    ...
    \item \textbf{Retire temporary controls after remediation.} Validate the
    fixed kernel, remove obsolete module or BPF policies, and confirm that
    normal application behaviour is restored. A successful workaround should
    not become permanent infrastructure by accident.
\end{enumerate}

These priorities do not change the present result. They identify where a future
study could convert bounded laboratory confidence into broader deployment
evidence. Until that work exists, MORI v2.7 remains a research-grade example of
selective behavioural prevention, not a production recommendation.

\subsection{Conclusion}

This project began with a published local privilege-escalation technique and
asked a larger question than whether it could print a root prompt. The isolated
laboratory reconstructed the vulnerable Ubuntu runtime, recorded an ordinary
UID~1001 starting state, and directly validated the transition to UID~0. It then
separated that decisive result from the accompanying mounted-view anomaly,
showing that the normal VFS path returned changed content while a direct ext4
read retained the original digest and that reboot restored the mounted view.

The observation phase turned that effect into a behavioural integrity signal.
MORI detected the protected target's deviation without binding the decision to
one proof-of-concept filename or hash, but did not claim unique attribution or
prevention. The control phase then made the trade-off between breadth and
selectivity visible. Broad module denial interrupted the known path at the cost
of the tested interface. MORI v2.7 preserved the exercised interface operations
and denied the two tested correlated exploit paths, but only within a documented
process, lifecycle, and deployment model.

The vendor comparison prevented the custom result from becoming the wrong final
answer. Canonical's packaged module rule remained the preferable broad emergency
measure when temporary interface loss was acceptable. More importantly, neither
temporary control replaced the corrected kernel. On the final patched runtime,
the affected module loaded, the tested benign operations remained reachable,
both proof-of-concept implementations failed to reproduce the prior privilege
transition, and \code{/usr/bin/su} retained its known-good captured state.
...
for the demonstrated path, but neither uniquely attributed every event
nor provided universal protection.

The corrected vendor kernel removed the reproduced behaviour while
preserving the tested AF_ALG interface operations. It is the appropriate
final remediation for the tested system.
\end{lstlisting}

That statement is deliberately narrower than a universal security claim. The
project used one principal vulnerable VM, one Ubuntu kernel line, two preserved
proof-of-concept implementations, a bounded target set, and a finite regression
suite. It did not reproduce a container escape, prove every exploit variant,
measure a population-level success rate, or establish production readiness for
the custom control. Those limits are part of the result rather than exceptions
to it.

The lasting output is therefore not only the UID~0 screenshot, the MORI deny
message, or the patched-side failure. It is the complete evidence-linked path
between them: vulnerable state, demonstrated impact, observable consequence,
temporary control, vendor comparison, and validated remediation. The project
ends where temporary controls should end---with the underlying flaw corrected,
the tested functionality restored, and the remaining uncertainty stated
openly.