...
\begin{evidencebox}
Phase~03 is preserved under \path{evidence/03-detection/}, with the exact-hash
comparison under \path{evidence/03-detection/yara-comparison/}. The main
manifest lists 22 objects, of which 21 verify; the nested YARA manifest lists
eight objects, of which seven verify. In both sets, the only failed digest is
the corresponding \path{README.md}. All 28 experimental artifacts, detector
files, rule files, and screenshots verify and are the evidence used below.
\end{evidencebox}
...
\begin{figure}[H]
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{03-detection/screenshots/05-baseline-tamper-denied.png}
    \caption{Attempted ordinary-user modification of the protected baseline.
    The write was denied, and the root-owned file retained its recorded mode,
    size, and SHA-256 digest.}
    \label{fig:phase03-baseline-tamper-denied}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{03-detection/screenshots/10-mori-system-service-clean-state.png}
    \caption{Journal capture after MORI was restarted as a system service. It
    records the clean initial state, the 12 protected paths, the baseline path,
    and the configured 0.25-second polling interval.}
    \label{fig:phase03-mori-clean-service}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{03-detection/screenshots/12-mori-detection-with-root-confirmation.png}
    \caption{Controlled monitored execution. MORI reports the
    \code{/usr/bin/su} digest mismatch in the journal while the adjacent
    unprivileged session records the resulting UID~0 identity.}
    \label{fig:phase03-mori-detection-root}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{03-detection/yara-comparison/screenshots/00-yara-known-poc-match.png}
    \caption{The exact-hash YARA rule matches the original known laboratory
    sample. The sample was scanned, not executed, for this comparison.}
    \label{fig:phase03-yara-known-match}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{03-detection/yara-comparison/screenshots/01-yara-harmless-variant-no-match.png}
    \caption{Exact-sample comparison after appending one trailing newline. The
    resulting 732-byte variant received a different SHA-256 digest and did not
    match the rule; it was deliberately not executed.}
    \label{fig:phase03-yara-variant-no-match}
\end{figure}
...
MORI detects an integrity violation consistent with Copy Fail, not unique
attribution. Chapter~7 therefore evaluates a compensating control designed to
interrupt the tested sequence before privilege is gained.