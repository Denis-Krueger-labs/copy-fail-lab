...
The mechanism described in this chapter comes from the published Copy Fail
analysis and the upstream kernel fix. The later experiment asks a narrower
question: do the states and effects observed in the laboratory match that
explanation? The recorded results can answer that question without being
treated as an instruction-by-instruction trace of the complete kernel path.
...
dirty for normal filesystem writeback. Under the published mechanism, the
cached representation changes without a conventional persistent write to the
underlying file. The result is therefore a deterministic four-byte write into
the page cache of a file that the attacker only needs permission to read
\cite{copyfail-research}.
...
image of the binary. The file remains owned by root and retains its setuid
metadata; the exploit only needs read access to poison the cached executable
image.
...
marked dirty for normal filesystem writeback. This makes the modification
volatile rather than a conventional persistent file write, but that does not
reduce its immediate impact: the attacker only needs the modified executable to
run once to turn the page-cache write into a root shell
\cite{copyfail-research}. The laboratory later checks this distinction by
comparing the mounted view with a direct read through the selected backing path
and by recording the state after a reboot. These observations support the
transient behaviour seen in the tested VM without claiming to trace every cache,
block, and storage layer.

The impact is also not necessarily limited to processes running directly on the
host. Canonical notes that the same vulnerability may facilitate container
escape scenarios in affected environments \cite{ubuntu-copyfail-cve}. This
project does not reproduce a container escape, so that impact remains published
context rather than a laboratory result.
...
Disabling the vulnerable interface can stop the demonstrated path, but it does
so by removing functionality. The kernel patch instead removes the vulnerable
behaviour itself. Both approaches provide useful reference points when
evaluating whether additional detection or mitigation techniques can identify
or interrupt the tested Copy Fail path without simply disabling the affected
subsystem.
...
\begin{description}
    ...
    Can Copy Fail be reproduced in the isolated laboratory environment by an
    ordinary, unprivileged local user?

    \item[RQ2 Observability.]
    Which system-level behaviours can be observed while the vulnerability is
    being exploited, and which of these provide useful detection opportunities?
    ...
    kernel is patched while preserving the benign operations selected for
    control testing?
...
Each question is answered through one or more controlled experimental phases.
The following chapters develop those answers in the same order as the
laboratory workflow.
...
separate from the reference VM so that a known-good comparison state remains
available while the target moves through vulnerable, controlled, and patched
states.
...
integrity verification after sanitization. The public evidence package keeps
the corresponding paths, manifest results, screenshots, and known limitations
separate from the analytical narrative.
