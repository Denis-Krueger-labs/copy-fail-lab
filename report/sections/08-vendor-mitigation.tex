...
blocking ordinary loading of \code{algif\_aead}, whereas MORI v2.7 preserved
the tested interface operations by enforcing a narrower behavioural
correlation. RQ5 now places those results beside Canonical's temporary response:
...
\begin{evidencebox}
The Phase~01, Phase~04, and Phase~05 public manifests contain 14, 126, and 18
paths respectively. Every listed object passed an independent
\code{sha256sum -c} verification before this revision. The causal comparison
itself rests on the Phase~01 vendor-policy capture and the Phase~04 custom-control
experiments. Phase~05 is used only to define the transition into the
corrected-kernel retest.
\end{evidencebox}
...
\cite{ubuntu-copyfail-cve,ubuntu-copyfail-fixes}. Second, Phase~01 records the
...
Canonical's guidance of 30 April 2026 documents mitigation-bearing \code{kmod}
updates that blocked loading of the affected \code{algif\_aead} module while
fixed kernel packages were being deployed. For Ubuntu 24.04 LTS Noble, the
mitigation-bearing package version was \code{31+20240202-2ubuntu7.2}
\cite{ubuntu-copyfail-fixes}. Canonical's manual equivalent writes the following
\code{modprobe} install rule:
...
\begin{figure}[H]
    ...
    \caption{Phase~01 reference-system captures. The upper image records the
    already-corrected \code{6.8.0-137-generic} kernel and mitigation-bearing
    \code{kmod} package. The lower image records the installed
    \code{algif\_aead} rule and its \code{modprobe} dry-run resolution to
    \code{/bin/false}. Together they prove policy presence and resolution
    behaviour, not an isolated vulnerable-kernel exploit result.}
    \label{fig:phase01-vendor-kmod-reference}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{04-custom-mitigation/screenshots/00-mori-guard-configuration.png}
    \caption{Phase~04 MORI Guard v1 configuration and effective dry run on the
    vulnerable \code{6.8.0-116-generic} guest. Comparing this capture with
    \cref{fig:phase01-vendor-kmod-reference} shows the shared module-loading
    policy class: both replace ordinary \code{algif\_aead} insertion with a
    failing command, while the custom target adds laboratory-specific
    presentation and logging.}
    \label{fig:phase04-v1-guard-config-comparison}
\end{figure}
...
active. The complete denial and post-state are preserved in
\cref{fig:phase04-v1-poc-blocked}.
...
fundamental compatibility boundary. While the module is absent, both deny
ordinary load attempts from malicious and benign callers before either can use
it.
...
Those two differences are directly visible in the preceding chapter's Phase~04
captures: \cref{fig:phase04-v22-benign-bind} records the retained unprivileged
bind, while \cref{fig:phase04-v27-python-denied,fig:phase04-v27-c-denied}
record the final Python and independent C denials with their associated
telemetry and post-test state.
...
MORI v2.7:
    socket, bind, and accept retained; complete application
...
block the same ordinary loading path for benign callers. MORI Guard v1 adds
laboratory journalling and presentation, but it is locally maintained and
provides no compatibility advantage. Canonical's packaged rule is therefore the
stronger operational choice for broad temporary containment.
...
It also supplied structured decision telemetry. Those advantages are balanced
by a narrower behavioural coverage claim, more complex privileged deployment,
kernel-feature dependencies, possible false positives for a benign matching
sequence, and explicit evasion and fail-open boundaries.
